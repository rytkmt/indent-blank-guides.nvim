...
local function get_indent_namespace()
  return api.nvim_create_namespace('indent_blank_guides_neovim')
end
...
local indent_highlight_color =function ()
  ...
  api.nvim_command('hi IndentBlankGuidesEven guifg=' .. even['fg'] .. ' guibg='.. even['bg'])
  api.nvim_command('hi IndentBlankGuidesEvenVirtext guifg=' .. even['bg'] .. ' guibg='.. even['fg'])
  api.nvim_command('hi IndentBlankGuidesOdd guifg=' .. odd['fg'] .. ' guibg='.. odd['bg'])
  api.nvim_command('hi IndentBlankGuidesOddVirtext guifg=' .. odd['bg'] .. ' guibg='.. odd['fg'])
end
...
local render_indent_blank_guides = function()
...
local indent_blank_guides_enable = function()
  ...
  local indent_blank_guides = coroutine.create(render_indent_blank_guides)
  local indent_render = function()
    while true do
      local _,group,column_start,guide_size,indent_size = coroutine.resume(indent_blank_guides)
      ...
      if coroutine.status(indent_blank_guides) == 'dead' then
        break
      end
    end
  end
...
function M.indent_blank_guides_enable()
  if next(new_opts) == nil then
    new_opts = get_default_options()
  end
  if not new_opts.indent_enable then return end

  indent_blank_guides_enable()
  if not indent_enabled then
    M.indent_blank_guides_augroup()
  end
end

function M.indent_blank_guides_disable()
...
function M.indent_blank_guides_toggle()
  if indent_enabled then
    M.indent_blank_guides_disable()
  else
    indent_enabled = true
    M.indent_blank_guides_enable()
  end
end

local function setup_cli()
  -- command! -bar IndentBlankGuidesEnable lua require('indent_blank_guides').indent_blank_guides_enable()
  -- command! -bar IndentBlankGuidesDisable lua require('indent_blank_guides').indent_blank_guides_disable()
  -- command! -bar IndentBlankGuidesToggle lua require('indent_blank_guides').indent_blank_guides_toggle()

  api.nvim_create_user_command('IndentBlankGuidesEnable', function()
    M.indent_blank_guides_enable()
  end, {})
  api.nvim_create_user_command('IndentBlankGuidesDisable', function()
    M.indent_blank_guides_disable()
  end, {})
  api.nvim_create_user_command('IndentBlankGuidesToggle', function()
    M.indent_blank_guides_toggle()
  end, {})
end

function  M.indent_blank_guides_augroup()
  api.nvim_command('augroup indent_blank_guides_nvim')
  api.nvim_command('autocmd!')
